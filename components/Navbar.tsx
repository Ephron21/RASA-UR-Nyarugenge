...
const Navbar: React.FC<NavbarProps> = ({ user, departments, onLogout }) => {
  ...
  // Point to a local asset. User should place logo.png in public folder.
      ...
      <div className="max-container flex items-center justify-between px-4">
        ...
        <Link to="/" className="flex items-center gap-4 group">
          <motion.div 
            ...
            className="relative h-14 w-auto flex items-center justify-center"
          >
            {/* Elegant Fallback: This shows 'R' if logo.png is missing or loading */}
            <div className="absolute h-12 w-12 bg-cyan-500 rounded-2xl flex items-center justify-center text-white font-black text-2xl shadow-lg transition-opacity duration-500 group-hover:rotate-[360deg]">
              R
            </div>
            ...
            <img 
              ...
              onLoad={(e) => {
                // When image loads, hide the fallback and show the image
              ...
              onError={(e) => {
                // If image fails, keep fallback visible and hide the broken img tag
                (e.target as any).style.display = 'none';
              }}
            />
          </motion.div>
          <div className="flex flex-col">
            <span className={`font-black text-2xl tracking-tighter leading-none ${navTextColor}`}>
              RASA <span className="text-cyan-500">NYG</span>
            </span>
            <span className={`text-[7px] font-black uppercase tracking-[0.3em] mt-1 ${scrolled || !isHomePage ? 'text-gray-400' : 'text-white/60'}`}>
                ...
                <div className="w-16 h-16 rounded-2xl bg-cyan-500 overflow-hidden shadow-xl border-2 border-white shrink-0">
                  {user.profileImage ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-white">
                      {user.role === 'it' ? <Shield size={24}/> : <UserIcon size={24} />}
                    </div>
                  )}
                </div>
                <div>
